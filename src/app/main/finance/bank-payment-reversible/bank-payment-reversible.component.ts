...
@Component({
  selector: "app-bank-payment-reversible",
  templateUrl: "./bank-payment-reversible.component.html",
})
export class BankPaymentReversibleComponent {
  ...
  rowSelection: string;
  @Input() rowData: any;
  bankpaymentReversibleForm: FormGroup;
  ...
  constructor(
    ...
    this.bankpaymentReversibleForm = this.fb.group({
  ...
  setPickerValue(value: any, title: string) {
    ...
    switch (title) {
      case "Location":
        this.bankpaymentReversibleForm.patchValue({
          userLocationId: value?.id,
          userLocationName: value.name,
        });
        this.getVoucherNumber(
          "BRR",
          this.bankpaymentReversibleForm.value.issueDate,
          value?.id
        );
        break;
      default:
  ...
  onDateChange(value?: any) {
    debugger;
    if (value) {
      this.bankpaymentReversibleForm.value.issueDate = value;
    }
    if (
      this.bankpaymentReversibleForm.value.userLocationId &&
      this.bankpaymentReversibleForm.value.issueDate
    ) {
      this.getVoucherNumber(
        "BRR",
  ...
  isFieldInvalid(field: string): boolean {
    const control = this.bankpaymentReversibleForm.get(field);
  ...
  getVoucherNumber(prefix: string, issueDate: string, userLocationId: number) {
    ...
    this._financeModuleService
      ...
      .subscribe({
        next: (response) => {
          debugger;
          this.bankpaymentReversibleForm
            .get("voucherNumber")
            .setValue(response);
        },
        error: (err) => {
  ...
  save() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        debugger;
        this._financeModuleService
          .create(this.bankpaymentReversibleForm.value, this.target)
          .pipe(
            finalize(() => {
              this.saving = false;
            }),
  ...
  update() {
    ...
    const updateData = {
      ...this.bankpaymentReversibleForm.value,
  ...
  show(id?: number) {
    ...
    if (id) {
        ...
        .subscribe({
          next: (response) => {
            ...
            this.bankpaymentReversibleForm
              ...
              .setValue(this.dataForEdit.generalPaymentId);
      ...
      this.rowData = [];
      this.displayModal = true;
      this.bankpaymentReversibleForm.reset();
      this.bankpaymentReversibleForm.enable();
      this.bankpaymentReversibleForm.get("issueDate").setValue(this.today);
  ...
  viewOnly(id: any) {
    ...
    this.isEditMode = false;
    this.show(id);
    this.bankpaymentReversibleForm.disable();
    this.MinDate = null;
  }
  ...
  GetBankPayment() {
    this._financeModuleService
      .getAll("GeneralPayment", { linkedDocument: 3 })
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          debugger;
