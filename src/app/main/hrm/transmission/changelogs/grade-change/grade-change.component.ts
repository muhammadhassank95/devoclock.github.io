...
@Component({
  selector: "app-grade-change",
  templateUrl: "./grade-change.component.html",
})
export class GradeChangeComponent {
  protected gridApi: GridApi;
  rowData: any[] = [];
  transitionInputsDto: TransitionInputsDto = new TransitionInputsDto();
  employeeDropdown: any[] = [];
  employeeSelectedItem: any;
  currentGradeDropdown: any[] = [];
  currentGradeSelectedItem: any;
  newGradeDropdown: any[] = [];
  newGradeSelectedItem: any;
  constructor(
  ...
  ngOnInit(): void{
    ...
    this.fetchDropdownData('Grade').subscribe((items) => (this.currentGradeDropdown = items));
    this.fetchDropdownData('Grade').subscribe((items) => (this.newGradeDropdown = items));
  }
  ...
  onDropdownChange(event: any, type: string){
    ...
    switch (type) {
      case "Employee":
        ...
        this.getCurrentGrade(value.additional)
        break;
      case "current":
  ...
  save() {
    ...
    if (!this.transitionInputsDto.currentId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select Current Grade",
        life: 2000,
      });
      return;
    }
    if (!this.transitionInputsDto.newId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select New Grade",
        life: 2000,
      });
      return;
    }
    ...
    this._transmitionService
      .create(this.transitionInputsDto, "UpdateGrade")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  getCurrentGrade(id: any) {
    this._transmitionService.getCurrentInfo(id, "GetGrade").subscribe({
      next: (response) => {
        debugger;
        if (response) {
