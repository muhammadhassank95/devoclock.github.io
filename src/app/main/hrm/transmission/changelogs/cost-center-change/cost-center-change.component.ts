...
@Component({
  selector: "app-cost-center-change",
  templateUrl: "./cost-center-change.component.html",
})
export class CostCenterChangeComponent {
  protected gridApi: GridApi;
  rowData: any[] = [];
  transitionInputsDto: TransitionInputsDto = new TransitionInputsDto();
  employeeDropdown: any[] = [];
  employeeSelectedItem: any;
  currentCostCenterDropdown: any[] = [];
  currentCostCenterSelectedItem: any;
  newCostCenterDropdown: any[] = [];
  newCostCenterSelectedItem: any;
  constructor(
  ...
  ngOnInit(): void{
    ...
    this.fetchDropdownData('CostCenter').subscribe((items) => (this.currentCostCenterDropdown = items));
    this.fetchDropdownData('CostCenter').subscribe((items) => (this.newCostCenterDropdown = items));
  }
  ...
  onDropdownChange(event: any, type: string){
    ...
    switch (type) {
      case "Employee":
        ...
        this.getCurrentCostCenter(value.additional)
        break;
      case "current":
  ...
  save() {
    ...
    if (!this.transitionInputsDto.currentId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select Current CoastCenter",
        life: 2000,
      });
      return;
    }
    if (!this.transitionInputsDto.newId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select New CoastCenter",
        life: 2000,
      });
      return;
    }
    ...
    this._transmitionService
      .create(this.transitionInputsDto, "UpdateCostCenter")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  getCurrentCostCenter(id: any) {
    this._transmitionService.getCurrentInfo(id, "GetCostCenter").subscribe({
      next: (response) => {
        debugger;
        if (response) {
