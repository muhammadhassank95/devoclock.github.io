...
export class ShiftChangeComponent {
  protected gridApi: GridApi;
  rowData: any[] = [];
  transitionInputsDto: TransitionInputsDto = new TransitionInputsDto();
  employeeDropdown: any[] = [];
  employeeSelectedItem: any;
  currentShiftDropdown: any[] = [];
  currentShiftSelectedItem: any;
  newShiftDropdown: any[] = [];
  newShiftSelectedItem: any;
  constructor(
  ...
  ngOnInit(): void{
    ...
    this.fetchDropdownData('Shift').subscribe((items) => (this.currentShiftDropdown = items));
    this.fetchDropdownData('Shift').subscribe((items) => (this.newShiftDropdown = items));
  }
  ...
  onDropdownChange(event: any, type: string){
    ...
    switch (type) {
      case "Employee":
        ...
        this.getCurrentShift(value.additional)
        break;
      case "current":
  ...
  save() {
    ...
    if (!this.transitionInputsDto.currentId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select Current Shift",
        life: 2000,
      });
      return;
    }
    if (!this.transitionInputsDto.newId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select New Shift",
        life: 2000,
      });
      return;
    }
    ...
    this._transmitionService
      .create(this.transitionInputsDto, "UpdateShift")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  getCurrentShift(id: any) {
    this._transmitionService.getCurrentInfo(id, "GetShift").subscribe({
      next: (response) => {
        debugger;
        if (response) {
