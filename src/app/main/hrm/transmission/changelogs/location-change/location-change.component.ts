...
export class LocationChangeComponent {
  ...
  currentLocationDropdown: any[] = [];
  currentLocationSelectedItem: any;
  newLocationDropdown: any[] = [];
  newLocationSelectedItem: any;
  constructor(
  ...
  ngOnInit(): void{
    ...
    this.fetchDropdownData('Location').subscribe((items) => (this.currentLocationDropdown = items));
    this.fetchDropdownData('Location').subscribe((items) => (this.newLocationDropdown = items));
  }
    ...
    switch (type) {
      case "Employee":
        ...
        if (value.additional) {
          ...
          this.getCurrentLocation(value.additional);
        }
        break;
      case "current":
  ...
  save() {
    ...
    if (!this.transitionInputsDto.currentId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select Current Location",
        life: 2000,
      });
      return;
    }
    if (!this.transitionInputsDto.newId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select New Location",
        life: 2000,
      });
      return;
    }
    ...
    this._transmitionService
      .create(this.transitionInputsDto, "UpdateLocation")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  getCurrentLocation(id: any) {
    this._transmitionService.getCurrentInfo(id, "GetLocation").subscribe({
      next: (response) => {
        debugger;
        if (response) {
