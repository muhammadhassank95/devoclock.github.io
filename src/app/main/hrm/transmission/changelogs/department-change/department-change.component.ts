...
export class DepartmentChangeComponent {
  protected gridApi: GridApi;
  rowData: any[] = [];
  transitionInputsDto: TransitionInputsDto = new TransitionInputsDto();
  employeeDropdown: any[] = [];
  employeeSelectedItem: any;
  currentDepartmentDropdown: any[] = [];
  currentDepartmentSelectedItem: any;
  newDepartmentDropdown: any[] = [];
  newDepartmentSelectedItem: any;
  constructor(
  ...
  ngOnInit(): void{
    ...
    this.fetchDropdownData('Department').subscribe((items) => (this.currentDepartmentDropdown = items));
    this.fetchDropdownData('Department').subscribe((items) => (this.newDepartmentDropdown = items));
  }
  ...
  onDropdownChange(event: any, type: string){
    ...
    switch (type) {
      case "Employee":
        ...
        this.getCurrentDepartment(value.additional)
        break;
      case "current":
  ...
  save() {
    ...
    if (!this.transitionInputsDto.currentId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select Current Department",
        life: 2000,
      });
      return;
    }
    if (!this.transitionInputsDto.newId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select New Department",
        life: 2000,
      });
      return;
    }
    ...
    this._transmitionService
      .create(this.transitionInputsDto, "UpdateDepartment")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  getCurrentDepartment(id: any) {
    this._transmitionService.getCurrentInfo(id, "GetDepartment").subscribe({
      next: (response) => {
        debugger;
        if (response) {
