...
export class TransmissionComponent extends AppComponentBase implements OnInit {
    ...
    currentDesignationDropdown: any[] = [];
    currentDesignationSelectedItem: any;
    newDesignationDropdown: any[] = [];
    newDesignationSelectedItem: any;
    constructor(
        injector: Injector,
    ...
    ngOnInit(): void {
        ...
        this.fetchDropdownData('Designation').subscribe((items) => (this.currentDesignationDropdown = items));
        this.fetchDropdownData('Designation').subscribe((items) => (this.newDesignationDropdown = items));
    }
      ...
      onDropdownChange(event: any, type: string){
        ...
        switch (type) {
            case "Employee":
                ...
                this.getCurrentDesignation(value.additional)
                break;
            case "current":
  ...
  save() {
    ...
    if (!this.transitionInputsDto.currentId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select Current Designation",
        life: 2000,
      });
      return;
    }
    if (!this.transitionInputsDto.newId) {
      this.messageService.add({
        // severity: "success",
        summary: "Required",
        detail: "Please Select New Designation",
        life: 2000,
      });
      return;
    }
    ...
    this._transmitionService
      .create(this.transitionInputsDto, "UpdateDesignation")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  getCurrentDesignation(id: any) {
    this._transmitionService.getCurrentInfo(id, "GetDesignation").subscribe({
      next: (response) => {
        debugger;
        if (response) {
